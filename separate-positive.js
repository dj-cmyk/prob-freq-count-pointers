...
function swap(arr, idx, revIdx) {
    temp = arr[idx]
    arr[idx] = arr[revIdx]
    arr[revIdx] = temp
}

function separatePositive(nonZeroIntArray) {
    let revIdx = 0
    for (let i=0; i< nonZeroIntArray.length; i++) {
        if (nonZeroIntArray[i] > 0) {
            swap(nonZeroIntArray, i, revIdx)
            revIdx++;
            console.log(nonZeroIntArray)
        } 
    }
    return nonZeroIntArray 
